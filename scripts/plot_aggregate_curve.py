...
for _, row in df_clean.iterrows():
    ...
    # Key fix: normalize x to true position in full reasoning chain
    essp_proportion = row['essp_index'] / row['total_steps']
    x_original = np.linspace(0, essp_proportion, len(accs))
...
plt.title(f'Aggregate Sufficiency Curve (N={len(interpolated_y)})')
plt.legend()
plt.grid(True, alpha=0.3)
plt.xlim(0, 100)
plt.ylim(0, 1)
plt.tight_layout()
plt.savefig('aggregate_sufficiency.png', dpi=300)
plt.show()